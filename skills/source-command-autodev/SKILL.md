---
name: "source-command-autodev"
description: "Desarrollo autónomo por fases: toma un plan existente, crea ramas de trabajo, implementa con tests y deja PRs listos sin merge. Usar con /autodev o cuando el usuario pida ejecutar un plan completo de forma autónoma."
---

# source-command-autodev

Use this skill when the user asks to run the migrated source command `autodev`.

## Command Template

Ejecuta desarrollo autónomo basado en un plan de implementación existente. El usuario puede estar lejos del PC, así que debes ser completamente autónomo y no hacer preguntas a menos que encuentres un blocker crítico que impida continuar.
...
- Seguir las convenciones del proyecto (leer AGENTS.md del proyecto si existe)
...
- **NO hacer push directo a `master`, `main` o `develop`.** Trabajar siempre en ramas seguras.
- **NO usar worktrees** salvo pedido explícito del usuario.
...
- **Tests:** toda funcionalidad nueva y todo bug fix debe incluir tests nuevos o actualizar los existentes, salvo una instruccion mas especifica y vigente del proyecto.
...
- **AGENTS.md del proyecto:** si el subproyecto o el repo padre tiene AGENTS.md, leerlo y seguir sus convenciones (idioma de commits, branch naming, etc.).
