...
description: >-
  ...
  "consultar Grok", or "revisa con x.ai". Choose one reviewer per run: Codex via
  `codex exec -m gpt-5.6-sol -c model_reasoning_effort="xhigh" --sandbox read-only` by default;
  Claude Code via the `opus` or `fable` preset; OpenCode via GLM-5.2 max; Grok Build CLI via Grok 4.5; or
  Cursor in read-only ask mode. Use to challenge assumptions and detect scenarios where a
  conclusion fails. Do not use for small obvious edits.
---
...
2. The bundled script invokes exactly one reviewer:
   ...
   - `-Reviewer claude`: Claude Code with `-ClaudePreset opus` (`--model claude-opus-4-8`, `--effort max`) by default, or explicit `-ClaudePreset fable` (`--model claude-fable-5`, `--effort high`), `--permission-mode plan`, `--no-session-persistence`, empty strict MCP config, temp working directory, and `--add-dir` pointing at the invocation repo. Only `Read`, `LS`, `Glob`, and `Grep` tools are enabled. It does not use `--bare` by default because Claude Code bare mode cannot use the normal claude.ai/keychain login.
...
- `DIFF_REVIEW_CLAUDE_MODEL`: derived from the preset (`claude-fable-5` for `fable`, `claude-opus-4-8` for `opus`) unless explicitly overridden.
